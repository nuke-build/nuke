...
namespace Nuke.Common.Tools.Xunit
{
    ...
    {
        public static void Xunit2 (IEnumerable<string> assemblyFiles, XunitSettings xunitSettings = null, ProcessSettings processSettings = null)
        {
            Xunit2(x => (xunitSettings ?? x).AddTargetAssemblies(assemblyFiles), processSettings);
        }

        private static void AssertProcess (IProcess process, XunitSettings xunitSettings)
        {
