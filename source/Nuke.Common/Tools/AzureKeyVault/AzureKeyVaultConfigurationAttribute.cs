    ...
    public class AzureKeyVaultConfigurationAttribute : ValueInjectionAttributeBase
    {
        ...
        public override object GetValue(MemberInfo member, object instance)
        {
            return new AzureKeyVaultConfiguration
                   {
                       BaseUrl = BaseUrl ?? EnvironmentInfo.GetParameter<string>(BaseUrlParameterName),
                       TenantId = TenantId ?? EnvironmentInfo.GetParameter<string>(TenantIdParameterName),
                       ClientId = ClientId ?? EnvironmentInfo.GetParameter<string>(ClientIdParameterName),
                       ClientSecret = EnvironmentInfo.GetParameter<string>(ClientSecretParameterName)
                   };
        }
