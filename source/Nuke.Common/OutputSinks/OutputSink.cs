...
namespace Nuke.Common.OutputSinks
{
    ...
    public abstract class OutputSink
    {
        ...
        protected virtual void WriteSummaryTable(NukeBuild build)
        {
            ...
            static string ToMinutesSecondsMilliSeconds(TimeSpan duration)
                => $"{(int) duration.TotalMinutes}:{duration:ss}.{duration:fffff}";
            ...
            {
                var line = CreateLine(target.Name, target.Status.ToString(), ToMinutesSecondsMilliSeconds(target.Duration), target.SkipReason);
            ...
            WriteNormal(new string(c: '─', count: allColumns));
            WriteInformation(CreateLine("Total", "", ToMinutesSecondsMilliSeconds(totalDuration)));
            WriteNormal(new string(c: '═', count: allColumns));
        }
