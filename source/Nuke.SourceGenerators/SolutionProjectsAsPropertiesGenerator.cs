    ...
    public class SolutionProjectsAsPropertiesGenerator : ISourceGenerator
    {
        ...
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            foreach (var (member, attributeData) in membersWithAttributes)
            {
                ...
                var classDeclaration = GetSolutionFolderDeclaration(nameof(Solution), solution.SolutionFolders, solution.Projects, isSolution: true);
            ...
            bool isSolution = false)
        {
            ...
            string GetSolutionFolderTypeName(string name)
                => $"{nameof(SolutionFolder)}_{GetMemberName(name)}";

            MemberDeclarationSyntax GetSolutionFolderPropertyDeclaration()
                => isSolution
                    ? ParseMemberDeclaration("private Solution SolutionFolder => this;")
        ...
        private static string GetSolutionFileFromParametersFile(AbsolutePath rootDirectory, string memberName)
        {
            ...
            return Path.Combine(rootDirectory, memberProperty.NotNull().Value<string>());
        }
